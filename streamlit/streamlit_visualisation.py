...
if selected == "Étude « Powerlines »":
    ...
    st.markdown('Quand on l\'evolution de ces feu par état, en se concentrant sur les états qui rapportent plus de 500 '
                'feux, on constate que le texas est le plus gros contributeur de cette anomalies, et de loin.')
    ...
    with col21:
        if selected_state == Top_state[0]:
            ...
            plt.legend();
            st.pyplot(fig)
            st.markdown('L\'échelle est tellement ecrasée que les précédent enregistrement du texas ne sont pas visible avant '
                        'l\'anomalie, la moyenne inter-état est sur le 0 tellement les écarts de valeurs sont important.')
    ...
    with st.expander('### Un réseau exposé'):
        st.markdown('Le texas est principalement équipé de ligne aérienne du fait de son réseau ancien et du coût important'
                    ' des lignes enterrées. La majorité du réseau texans est donc très exposé aux incidents qui déclenchent'
                    ' les incendies. ')
        st.markdown('**Les ligne tombées au sol** – Les lignes électriques peuvent tomber au sol (pour de nombreuses raisons), '
        'les disjoncteurs devraient se déclencher, mais dans 30 % des cas, ils consomment trop peu de courant pour '
        'fonctionner correctement et la ligne peut arquer pendant une longue période avant que le courant soit coupé sur '
        'la ligne.')
        st.markdown('**Les contact avec la végétation** – Les arbres et autres végétaux qui s\'immiscent dans les lignes électriques '
        ...
        st.markdown('**Un contacte entre les caténaires** – Les caténaires doivent être suffisamment espacés mais, dans certaines '
    ...
    with st.expander('### Le Texas a son propre réseau électrique'):
        ...
        'l\'une des raisons de la panne massive de 2021. Ainsi, les infrastructures électriques ne sont pas contrôlées au '
    ...
    with st.expander('### Des infrastructures vieillissantes'):
        ...
        'qu\'ils font le minimum pour éviter une défaillance systémique, la plupart du temps c\'est suffisant, mais en cas '
    ...
    with st.expander('### 2011'):
        ...
        st.markdown('On voit ici très bien la sécheresse qui a été classée pire sècheresse des 127 dernières années.')

    st.markdown('## Conclusion')
    st.markdown('On peu ici laisser de côté un problème dans l\'enregistrement des donnée, car en dehors de la période '
                'de sécheresse l\'anomalie disparait. ')
    st.markdown('Ce qui nous laisse une conjonction de facteurs pouvants expliquer l\'anomalie :')
    st.markdown('- Des standard de gestion du réseau électrique très différent du reste de l\'Union')
    st.markdown('- Un sècheresse parmi les plus dures jamais enregistrée')
    st.markdown('- Des equipments vieillissants maintenant dans les conditions tout juste satisfaisante pour le '
                'maintient de l\'activité dans les condition d\'opération normales')
    st.markdown('On peu craindre que les phénomènes exceptionnels se multipliants ce type d\'anomalie sera amené à '
                'se reproduire')
